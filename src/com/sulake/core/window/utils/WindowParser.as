package com.sulake.core.window.utils
{
   ...
   public class WindowParser implements IWindowParser
   {
      ...
      public static const const_970:String = "_EXCLUDE";
      
      public static const const_1693:String = "_INCLUDE";
      
      public static const const_1720:String = "_TEMP";
      ...
      protected var var_1145:Dictionary;
      
      protected var var_915:Dictionary;
      
      protected var var_1389:Dictionary;
      
      protected var var_1846:Map;
      
      protected var var_17:IWindowContext;
      ...
      public function WindowParser(param1:IWindowContext)
      {
         ...
         TypeCodeTable.fillTables(this._typeStringToCodeTable,this.var_1145);
         this.var_915 = new Dictionary();
         this.var_1389 = new Dictionary();
         ParamCodeTable.fillTables(this.var_915,this.var_1389);
         this.var_1846 = new Map();
      }
      
      private static function trimWhiteSpace(param1:String) : String
      {
         param1 = param1.replace(const_1130,"");
      ...
      public function dispose() : void
      {
         ...
         if(!this._disposed)
         {
            ...
            for(_loc1_ in this.var_1145)
            {
               delete this.var_1145[_loc1_];
            }
            for(_loc1_ in this.var_915)
            {
               delete this.var_915[_loc1_];
            }
            for(_loc1_ in this.var_1389)
            {
               delete this.var_1389[_loc1_];
            }
            this.var_1846.dispose();
            this.var_1846 = null;
            this.var_17 = null;
            this._disposed = true;
         }
      ...
      private function parseSingleWindowEntity(param1:XML, param2:WindowController, param3:Map) : IWindow
      {
         ...
         visible = this.parseAttribute(xml,"visible",variables,visible.toString()) == "HabboWindowManagerCom_habbo_window_alert_xml";
         ...
         window = this.var_17.create(name,null,type,style,params,rect,null,parent is IIterable ? null : parent,id,this.parseProperties(xml.child("variables")[0]),tags) as WindowController;
         ...
         background = this.parseAttribute(xml,"background",variables,window.background.toString()) == "HabboWindowManagerCom_habbo_window_alert_xml";
         blend = Number(this.parseAttribute(xml,"blend",variables,window.blend.toString()));
         clipping = this.parseAttribute(xml,"clipping",variables,window.clipping.toString()) == "HabboWindowManagerCom_habbo_window_alert_xml";
            ...
            if(parent != null)
            {
               if(parent is IIterable)
               {
                  if(window.x != rect.x || window.y != rect.y || window.width != rect.width || window.height != rect.height)
                  {
                     ...
                     if((params & 0) == WindowParam.const_181)
                     {
                        window.y = rect.y;
                     }
                  }
      ...
      public function windowToXMLString(param1:IWindow) : String
      {
         ...
         var _loc2_:String = "";
         var _loc3_:String = this.var_1145[param1.type];
         ...
         if(_loc7_ is IIterable)
         {
            ...
            if(_loc9_ > 0)
            {
               ...
               while(_loc12_ < _loc9_)
               {
                  ...
                  if(_loc10_.tags.indexOf(WindowParser.const_970) == -1)
                  {
