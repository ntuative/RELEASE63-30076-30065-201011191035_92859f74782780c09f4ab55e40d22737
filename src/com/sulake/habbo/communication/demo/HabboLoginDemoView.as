package com.sulake.habbo.communication.demo
{
   ...
   public class HabboLoginDemoView extends EventDispatcher
   {
      ...
      public static const const_507:String = "InitConnection";
       
      
      private var var_153:HabboCommunicationDemo;
      
      private var _window:IWindowContainer;
      
      private var var_154:SSOTicket;
      ...
      {
         this.var_153.windowManager.removeWindow("habbo_login_window");
      }
      ...
      {
         var _loc1_:XmlAsset = (this.var_153 as Component).assets.getAssetByName("login_window") as XmlAsset;
         if(_loc1_ == null)
         {
            Logger.log("Could  not find embedded Window XML for Login Window, login_window");
            return;
         }
         this._window = this.var_153.windowManager.createWindow("habbo_login_window","Login",HabboWindowType.const_129,HabboWindowStyle.const_38,0 | 0,new Rectangle(10,10,320,240),null) as IFrameWindow;
         var _loc2_:XML = _loc1_.content as XML;
         var _loc3_:IWindowContainer = this.var_153.windowManager.buildFromXML(_loc2_) as IWindowContainer;
         ...
         var _loc7_:SharedObject = SharedObject.getLocal(this.const_1507,"/");
         if(_loc5_ != null)
         {
         ...
         if(this.var_153.habboConfiguration)
         {
            if(this.var_153.habboConfiguration.getKey("use.sso","false") == "HabboWindowManagerCom_habbo_window_alert_xml")
            {
               _loc8_.select();
            }
         }
      ...
      public function populateUserList(param1:Map) : void
      {
         ...
         var _loc3_:XmlAsset = this.var_153.assets.getAssetByName("login_user_list_item") as XmlAsset;
         var _loc4_:IWindow = this.var_153.windowManager.buildFromXML(_loc3_.content as XML);
      ...
      private function windowEventProcessor(param1:WindowEvent = null, param2:IWindow = null) : void
      {
         ...
         if(_loc3_ != null)
         {
            ...
            _loc6_ = SharedObject.getLocal(this.const_1507,"/");
         ...
         {
            dispatchEvent(new Event(const_507));
         }
      }
      ...
      private function listEventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_153.sendTryLogin(this.name,this.password,param2.id);
      }
      ...
      private function initSSOTicket() : void
      {
         ...
         {
            this.var_154.removeEventListener(SSOTicket.const_323,this.onParsedTicketSuccess);
            this.var_154.removeEventListener(SSOTicket.const_292,this.onParsedTicketFailure);
            this.var_154 = null;
         }
         var _loc1_:String = "";
         if(this.var_153.habboConfiguration != null)
         {
            _loc1_ = this.var_153.habboConfiguration.getKey("url.prefix",_loc1_);
            _loc1_ = _loc1_.replace("http://","");
            _loc1_ = _loc1_.replace("https://","");
         }
         var _loc2_:IHabboWebLogin = this.var_153.habboCommunication.habboWebLogin(this.name,this.password);
         this.var_154 = new SSOTicket(this.var_153.assets,_loc2_,_loc1_);
         this.var_154.addEventListener(SSOTicket.const_323,this.onParsedTicketSuccess);
         this.var_154.addEventListener(SSOTicket.const_292,this.onParsedTicketFailure);
      }
      
      private function onParsedTicketSuccess(param1:Event) : void
      {
         Logger.log("Got ticket: " + this.var_154.ticket);
         this.var_153.shockwaveClientUrl = this.var_154.shockwaveClientUrl;
         this.var_153.ssoTicket = this.var_154.ticket;
         this.var_154.removeEventListener(SSOTicket.const_323,this.onParsedTicketSuccess);
         this.var_154.removeEventListener(SSOTicket.const_292,this.onParsedTicketFailure);
         this.var_154 = null;
         dispatchEvent(new Event(const_507));
