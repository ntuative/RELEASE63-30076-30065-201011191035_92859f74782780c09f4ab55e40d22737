package com.sulake.habbo.tracking
{
   ...
   public class LatencyTracker
   {
      ...
      {
         if(this.var_152 == null || this._communication == null || this._connection == null)
         {
            return;
         }
         this.var_1406 = int(this.var_152.getKey("latencytest.interval"));
         this.var_1409 = int(this.var_152.getKey("latencytest.report.index"));
         this.var_1878 = int(this.var_152.getKey("latencytest.report.delta"));
      ...
      public function update(param1:uint) : void
      {
         ...
         if(getTimer() - this.var_1407 > this.var_1406)
         {
            this.testLatency();
         }
      }
      ...
      {
         this.var_1407 = getTimer();
         this.var_504.add(this.var_1166,this.var_1407);
         this._connection.send(new LatencyPingRequestMessageComposer(this.var_1166));
         ++this.var_1166;
      }
      ...
      private function onPingResponse(param1:IMessageEvent) : void
      {
         ...
         var _loc10_:int = 0;
         var _loc11_:* = null;
         if(this.var_504 == null || this.var_187 == null)
         {
            return;
         }
         var _loc2_:LatencyPingResponseMessageParser = (param1 as LatencyPingResponseMessageEvent).getParser();
         var _loc3_:int = this.var_504.getValue(_loc2_.requestId);
         this.var_504.remove(_loc2_.requestId);
         var _loc4_:int = getTimer() - _loc3_;
         this.var_187.push(_loc4_);
         if(this.var_187.length == this.var_1409 && this.var_1409 > 0)
         {
            _loc5_ = 0;
            _loc6_ = 0;
            _loc7_ = 0;
            ...
            if(Math.abs(_loc9_ - this.var_1408) > this.var_1878 || this.var_1408 == 0)
            {
               this.var_1408 = _loc9_;
