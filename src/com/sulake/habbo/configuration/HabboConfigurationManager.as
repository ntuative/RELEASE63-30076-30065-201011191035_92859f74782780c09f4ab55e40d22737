package com.sulake.habbo.configuration
{
   ...
   public class HabboConfigurationManager extends Component implements IHabboConfigurationManager
   {
      ...
      private static var var_1348:String = "%";
      ...
      public function HabboConfigurationManager(param1:IContext, param2:uint = 0, param3:IAssetLibrary = null)
      {
         ...
         this.var_593 = this.getKey("url_prefix");
         if(this.var_593 != null && this.var_593.length > 0)
         {
            this.setKey("client.fatal.error.url",this.var_593 + "/flash_client_error");
         }
         this.init();
      }
      ...
      private function init() : void
      {
         ...
         if(!this.var_858)
         {
            ...
            _loc4_.addEventListener(AssetLoaderEvent.const_30,this.onInitConfiguration);
            _loc4_.addEventListener(AssetLoaderEvent.const_44,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event = null) : void
      {
         ...
         Core.error("Could not load external variables. HTTP status " + _loc3_ + ". Client startup failed!",true,Core.const_1448);
      }
      ...
      private function parseConfiguration(param1:String) : void
      {
         ...
         if(this.getKey("local.environment") == "HabboWindowManagerCom_habbo_window_alert_xml")
         {
            this.setKey("url.prefix",this.getKey("url.prefix.local"));
         }
         this.var_593 = this.getKey("url.prefix");
         if(this.var_593.length > 0)
         {
            externalAccessDomain = this.var_593.replace("http://","");
      ...
      {
         var _loc2_:String = this.var_1105[param1] as String;
         return _loc2_ != null;
      }
      
      public function getKey(param1:String, param2:String = "", param3:Dictionary = null) : String
      {
         var _loc4_:String = this.var_1105[param1] as String;
         if(_loc4_ == null)
         {
            _loc4_ = param2;
         }
         if(_loc4_ != null)
         {
            ...
            {
               _loc4_ = _loc4_.replace("%predefined%",this.var_593);
      ...
      public function setKey(param1:String, param2:String, param3:Boolean = false) : void
      {
         if(param3)
         {
            if(this.var_1105[param1] == null)
            {
               Logger.log(param1 + "=" + param2);
            }
         }
         if(this.var_1762.indexOf(param1) < 0)
         {
            this.var_1105[param1] = param2;
         }
      }
      ...
      private function fillParams(param1:String, param2:Dictionary) : String
      {
         ...
         while(_loc3_ < 10)
         {
            _loc4_ = param1.indexOf(var_1348);
            if(_loc4_ < 0)
            {
               break;
            }
            _loc5_ = param1.indexOf(var_1348,_loc4_ + 1);
            if(_loc5_ < 0)
            {
               break;
            }
