package com.sulake.habbo.session
{
   ...
   public class RoomSessionManager extends Component implements IRoomSessionManager, IRoomHandlerListener
   {
      ...
      override public function dispose() : void
      {
         ...
         if(this.var_192)
         {
            while(this.var_192.length > 0)
            {
               _loc1_ = this.var_192.getKey(0) as String;
               _loc2_ = this.var_192.remove(_loc1_) as RoomSession;
               if(_loc2_ != null)
               {
                  _loc2_.dispose();
               }
      ...
      private function initializeManager() : void
      {
         ...
         if(this.initialized && this.var_1503)
         {
            this.gotoRoom(this.var_2049,this.var_2050,this.var_1504);
            this.var_1503 = false;
            this.var_1504 = "";
         }
      }
      ...
      public function gotoRoom(param1:Boolean, param2:int, param3:String = "", param4:String = "") : Boolean
      {
         ...
         this.var_1207 = true;
         if(this.var_192.getValue(_loc6_) != null)
         {
         ...
         this.var_192.add(_loc6_,_loc7_);
         events.dispatchEvent(new RoomSessionEvent(RoomSessionEvent.const_216,_loc7_));
         return true;
      }
      
      public function startSession(param1:IRoomSession) : Boolean
      {
         if(param1.state == RoomSessionEvent.const_84)
         {
            return false;
         }
         if(param1.start())
         {
            this.var_1207 = false;
            events.dispatchEvent(new RoomSessionEvent(RoomSessionEvent.const_84,param1));
      ...
      public function sessionReinitialize(param1:int, param2:int, param3:int, param4:int) : void
      {
         ...
         var _loc6_:RoomSession = this.var_192.remove(_loc5_) as RoomSession;
         if(_loc6_ != null)
         {
            param4 = param2;
            _loc6_.reset(param3,param4);
            _loc5_ = this.getRoomIdentifier(param3,param4);
            _loc7_ = this.var_192.remove(_loc5_);
            if(_loc7_ != null)
            {
            }
            this.var_192.add(_loc5_,_loc6_);
      ...
      public function getSession(param1:int, param2:int) : IRoomSession
      {
         ...
         return this.var_192.getValue(_loc3_) as IRoomSession;
      }
      
      public function disposeSession(param1:int, param2:int) : void
      {
         var _loc3_:String = this.getRoomIdentifier(param1,param2);
         var _loc4_:RoomSession = this.var_192.remove(_loc3_) as RoomSession;
         if(_loc4_ != null)
         {
            events.dispatchEvent(new RoomSessionEvent(RoomSessionEvent.const_90,_loc4_));
            _loc4_.dispose();
         }
      }
