package com.sulake.habbo.window
{
   ...
   public class HabboWindowManagerComponent extends Component implements IHabboWindowManager, ICoreWindowManager, IWindowFactory, IUpdateReceiver, IWindowContextStateListener
   {
      ...
      private static const const_381:uint = 3;
      
      private static const const_1179:uint = 1;
      ...
      private var var_1276:IEventDispatcher;
      
      private var var_1044:Boolean = false;
      
      private var var_1043:ProfilerOutput;
      
      private var var_1278:ProfilerAgentTask;
      
      private var var_1277:ProfilerAgentTask;
      ...
      private function initialize(param1:IID, param2:IUnknown) : void
      {
         ...
         this._windowContextArray = new Array(const_381);
         ...
         var _loc6_:int = 0;
         while(_loc6_ < const_381)
         {
            this._windowContextArray[_loc6_] = new WindowContext("layer_" + _loc6_,this.var_826,this,this._localization,context.displayObjectContainer,_loc5_,this);
            _loc6_++;
         }
         this._windowContext = this._windowContextArray[const_1179];
         _loc7_ = new Bitmap(assets.getAssetByName("habbo_cursor_link_png").content as BitmapData);
         this._windowContext.getMouseCursor().defineCustomCursorType(MouseCursorType.const_365,_loc7_);
         _loc7_ = new Bitmap(assets.getAssetByName("habbo_cursor_drag_png").content as BitmapData);
         this._windowContext.getMouseCursor().defineCustomCursorType(MouseCursorType.const_1435,_loc7_);
         unlock();
      ...
      private function receiveProfilerInterface(param1:IID, param2:IUnknown) : void
      {
         ...
         if(_loc3_ != null)
         {
            if(!this.var_1043)
            {
               this.var_1043 = new ProfilerOutput(context,this,this.var_826);
            }
            this.var_1043.profiler = _loc3_;
            this.var_1278 = new ProfilerAgentTask("Update","Event processing");
            _loc3_.getProfilerAgentForReceiver(this).addSubTask(this.var_1278);
            this.var_1277 = new ProfilerAgentTask("Redraw","Window rasterizer");
            _loc3_.getProfilerAgentForReceiver(this).addSubTask(this.var_1277);
            this.var_1044 = true;
         }
      }
      ...
      {
         var _loc3_:Dictionary = this.var_363[param2] as Dictionary;
         if(_loc3_ == null)
         {
            return null;
         }
         var _loc4_:XML = _loc3_[param1] as XML;
         return _loc4_ == null ? this.var_363[0][param1] as XML : _loc4_;
      }
      ...
      {
         return this._windowContextArray[param1].getDesktopWindow().getChildAt(this._windowContext.getDesktopWindow().numChildren - 1);
      }
      ...
      public function update(param1:uint) : void
      {
         ...
         if(WindowContext.var_336.length > 0)
         {
            this.var_1276.dispatchEvent(new Event(HabboWindowTrackingEvent.HABBO_WINDOW_TRACKING_EVENT_INPUT));
            _loc2_ = const_381 - 1;
            while(_loc2_ >= 0)
            {
         ...
         this.var_1276.dispatchEvent(new Event(HabboWindowTrackingEvent.HABBO_WINDOW_TRACKING_EVENT_RENDER));
         _loc2_ = 0;
         while(_loc2_ < const_381)
         {
         ...
         if(WindowContext.var_336.length > 0)
         {
            WindowContext.var_336.flush();
         }
         this.var_1276.dispatchEvent(new Event(HabboWindowTrackingEvent.HABBO_WINDOW_TRACKING_EVENT_SLEEP));
      }
      ...
      public function mouseEventReceived(param1:String, param2:IWindow) : void
      {
         if(param2 != null)
         {
            ...
            {
               ErrorReportStorage.setParameter(HabboErrorVariableEnum.const_1333,new Date().getTime().toString());
            ...
            {
               ErrorReportStorage.setParameter(HabboErrorVariableEnum.const_1327,new Date().getTime().toString());
