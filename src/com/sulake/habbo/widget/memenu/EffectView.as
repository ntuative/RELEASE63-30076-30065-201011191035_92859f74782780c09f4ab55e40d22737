package com.sulake.habbo.widget.memenu
{
   ...
   public class EffectView
   {
      ...
      private var var_163:IWidgetAvatarEffect;
      
      private var var_863:IWindow;
      
      private var var_449:Number;
      ...
      private function onUpdate(param1:Event = null) : void
      {
         ...
         {
            if(this.var_163.isActive)
            {
               _loc2_ = this.var_163.secondsLeft / Number(this.var_163.duration);
               this.var_863.width = _loc2_ * this.var_449;
            }
            else
            {
               this.var_863.width = 0;
               this.var_176.stop();
            }
            this.setTimeLeft();
         }
      ...
      private function setTimeLeft() : void
      {
         ...
         {
            this.var_862.caption = "${widgets.memenu.effects.active.timeleft}";
            _loc1_ = this.var_862.text;
            if(this.var_163.isActive)
            {
               _loc2_ = this.var_163.secondsLeft;
               ...
               this.var_862.text = _loc1_;
            }
            else
            {
               this.var_862.caption = "${widgets.memenu.effects.activate}";
            }
         }
      }
      ...
      private function createWindow(param1:String) : void
      {
         ...
         var _loc7_:* = null;
         var _loc3_:String = "";
         if(this.var_163.isInUse)
         {
            _loc3_ = "memenu_effect_selected";
         }
         else if(this.var_163.isActive)
         {
         ...
         else
         {
            ...
            this.var_864 = this._window.findChildByName("effect_hilite");
            this.var_864.visible = false;
         }
         this.var_862 = this._window.findChildByName("time_left") as ITextWindow;
         this.setTimeLeft();
         this.var_863 = this._window.findChildByName("loader_bar");
         if(this.var_863 != null)
         {
            this.var_449 = this.var_863.width;
            this.var_176.start();
            this.onUpdate();
         }
         if(this.var_163.icon)
         {
            this.setElementBitmap("effect_icon",this.var_163.icon);
         }
      }
